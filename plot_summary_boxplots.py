...
if __name__ == "__main__":
    ...
    def save():
        ...
        plt.savefig("predictive_density__box.pdf")
        plt.savefig("predictive_density__box.png",dpi=350)
        plt.close()
    ...
    for n,qid in enumerate([10976, 10982, 10979, 11039, 10978, 10975, 10981]): # need to add canadan
        ...
        ypos = 6-n
        handle = ax.plot( [_10th, _90th ], [ypos]*2, lw=5, alpha=0.50 )
        ...
        if n in [0,1,4,5,6]:
            ...
            ax.text( median, ypos+0.02, "{:,.0f}".format( float(median)), va="bottom", ha="center", fontsize=10, color=clr, weight="bold" )
            ax.text( _10th , ypos+0.02, "{:,.0f}".format( float(_10th)) , va="bottom", ha="center", fontsize=10, color=clr, weight="bold" )
            ax.text( _90th , ypos+0.02, "{:,.0f}".format( float(_90th)) , va="bottom", ha="center", fontsize=10, color=clr, weight="bold" )
